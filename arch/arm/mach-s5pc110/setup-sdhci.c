...
void s3c6410_setup_sdhci2_cfg_gpio(struct platform_device *dev, int width)
{
	...
	/* Set all the necessary GPG2 pins to special-function 2 */
       ...
       s3c_gpio_cfgpin(S5PC11X_GPG2(0), S3C_GPIO_SFN(2));
	s3c_gpio_setpull(S5PC11X_GPG2(0), S3C_GPIO_PULL_UP);
    
	for (gpio = S5PC11X_GPG2(1); gpio < end; gpio++) {
...
unsigned int universal_sdhci2_detect_ext_cd(void)
{
	...
	return card_status ? 0 : 1;
       ...
       if(system_rev >= 0x08)
       {
              card_status = (readl(S5PC11X_GPH1DAT)) & (1 << 0); //victory.boot
        	printk(KERN_DEBUG " Universal : Card status  %d\n",card_status?1:0);
      		return card_status ? 1 : 0;
       }
       else
       {
	card_status = (readl(S5PC11X_GPH0DAT)) & (1 << 2); //victory.boot
	printk(KERN_DEBUG " Universal : Card status  %d\n",card_status?1:0);
...
void universal_sdhci2_cfg_ext_cd(void)
{
	...
	set_irq_type(IRQ_EINT(20), IRQ_TYPE_EDGE_BOTH);
       ...
       if(system_rev >= 0x08)
       {
        	err = gpio_request(S5PC11X_GPH1(0),"GPH10");

        	if (err){
        		printk("gpio request error : %d\n",err);
        	}else{
        		//s3c_gpio_setpull(S5PC11X_GPH1(0), S3C_GPIO_PULL_UP);
        	}
        	set_irq_type(IRQ_EINT8, IRQ_TYPE_EDGE_BOTH);
       }
       else
       {
	err = gpio_request(S5PC11X_GPH0(2),"GPH02");

	if (err){
		printk("gpio request error : %d\n",err);
	}else{
		//s3c_gpio_setpull(S5PC11X_GPH0(2), S3C_GPIO_PULL_UP);
	}
...
void universal_sdhci2_set_platdata(void)
{
	...
	s3c_hsmmc2_def_platdata.ext_cd = IRQ_EINT(2);
#else //victory.boot
       if(system_rev >= 0x08)
       {
          s3c_hsmmc2_def_platdata.ext_cd = IRQ_EINT8;
       }
       else
       {
	s3c_hsmmc2_def_platdata.ext_cd = IRQ_EINT2;
        }
#endif
