...
irqreturn_t gp2a_irq_handler(int irq, void *dev_id)
{
    ...
    if(system_rev < 0x0A) //jihyon.82.kim for victory rev 10
      value = gpio_get_value(S5PC11X_GPJ0(1));
    else
      value = gpio_get_value(S5PC11X_GPH0(1));
...
void gp2a_chip_init(void)
{
  ...
  if(system_rev < 0x0A) //jihyon.82.kim for victory rev 10
  {
	s3c_gpio_cfgpin(S5PC11X_GPJ0(1), S3C_GPIO_SFN(GPIO_PS_VOUT_AF));
	s3c_gpio_setpull(S5PC11X_GPJ0(1), S3C_GPIO_PULL_NONE);
	set_irq_type(S3C_GPIOINT(J0, 1), IRQ_TYPE_EDGE_BOTH);
   }
  else
  {
    s3c_gpio_cfgpin(S5PC11X_GPH0(1), S3C_GPIO_SFN(GPIO_PS_VOUT_AF));
	s3c_gpio_setpull(S5PC11X_GPH0(1), S3C_GPIO_PULL_NONE);
