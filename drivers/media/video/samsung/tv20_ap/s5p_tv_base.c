...
#if 0
        s3c_gpio_cfgpin(S5PC11X_GPJ4(4), S3C_GPIO_OUTPUT);//tv_en in lismore board
        s3c_gpio_setpull(S5PC11X_GPJ4(4), S3C_GPIO_PULL_UP);
        writel((readl(S5PC11X_GPJ4DAT)|(1<<4)),S5PC11X_GPJ4DAT);

        s3c_gpio_cfgpin(S5PC11X_GPJ1(4), S3C_GPIO_OUTPUT);//ear_sel in lismore board
        s3c_gpio_setpull(S5PC11X_GPJ1(4), S3C_GPIO_PULL_DOWN);

        s3c_gpio_setpull(S5PC11X_GPH3(6), S3C_GPIO_PULL_DOWN);
	printk("\n earsend 1  %x \n",readl(S5PC11X_GPH3DAT));


        writel(/*(readl(S5PC11X_GPJ1DAT)&(~(1<<4)))*/0x00,S5PC11X_GPJ1DAT);
        //
#endif
#if 0
int err = gpio_request(S5PC11X_GPJ4(4),"TV_EN");
udelay(50);
...
err = gpio_request(S5PC11X_GPJ1(4),"EAR_SEL");
udelay(50);
gpio_direction_output(S5PC11X_GPJ1(4),1);
gpio_set_value(S5PC11X_GPJ1(1),0);
udelay(50);
#endif
...
printk("+++++++++++++++++++++++hpd status is cable %s\n",
                s5ptv_status.hpd_status ? "inserted":"removed");



//	s5ptv_status.hpd_status = true; //!! :mkh
	...
	for (i = 0;i < S5P_TVMAX_CTRLS;i++) {
		...
		if (video_register_device(s5ptv_status.video_dev[i],
			...
			printk("\n****[mkh]:tvout driver registered successfully \n");
	}
//mkh:
