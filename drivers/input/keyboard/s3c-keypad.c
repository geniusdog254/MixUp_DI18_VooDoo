...
//hojun_kim 100511[
  ...
  //hojun_kim 100526 [
    ...
    { 
      ...
      key_status = (readl(S5PC11X_GPH0DAT)) & (1 << 5);
    }
    else
    {
      
  key_status = (readl(S5PC11X_GPJ1DAT)) & (1 << 4);
    }
  //hojun_kim ]  
...
//hojun_kim  
  if(OnOff && key_status)
  {
        s3c_gpio_cfgpin(S5PC11X_GPJ2(6), S3C_GPIO_INPUT);
        s3c_gpio_setpull(S5PC11X_GPJ2(6), S3C_GPIO_PULL_UP);
        printk("\n[Keypad LED]Turn ON\n");
  }
  else
  {
        s3c_gpio_cfgpin(S5PC11X_GPJ2(6), S3C_GPIO_INPUT);
        s3c_gpio_setpull(S5PC11X_GPJ2(6), S3C_GPIO_PULL_DOWN);
        printk("\n[Keypad LED]Turn OFF\n");
  }
}
//hojun_kim ]
...
//	printk("\n@@key_status=%d", key_status);
	if(key_status)
		input_report_key(dev,58,0);//hojun_kim changed powerkey code
	else
		input_report_key(dev,58,1);//hojun_kim changed powerkey code
...
//  s3c_gpio_cfgpin(S5PC11X_GPJ1(4), S3C_GPIO_SFN(0));
//hojun_kim 100526 [
  if(system_rev >= 0x08)
  { 

  s3c_gpio_cfgpin(S5PC11X_GPH0(5), S3C_GPIO_SFN(0));
  ...
  {
  s3c_gpio_setpull(S5PC11X_GPJ1(4), S3C_GPIO_PULL_NONE);
...
module_init(s3c_keypad_init);//hojun_kim for keypad
