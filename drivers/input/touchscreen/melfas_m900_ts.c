...
//victory.boot #include <linux/dprintk.h>

#define dprintk printk

#define MELFAS_I2C_ADDR 0x40
//#define IRQ_TOUCH_INT IRQ_EINT(8)
#define IRQ_TOUCH_INT S3C_GPIOINT(J0,5) //victory.boot
...
//			case 0x0C :
			...
			//victory.boot.temp if(!(keycode ==TOUCH_MENU && get_sending_oj_event()))
			if(!(keycode ==TOUCH_MENU))
			{
				if(button & 0x08) 
					keypress = 0;
				else 
...
//	set_irq_type(IRQ_TOUCH_INT, IRQ_TYPE_LEVEL_LOW);
	...
	if (gpio_is_valid(GPIO_TOUCH_EN)) {
		if (gpio_request(GPIO_TOUCH_EN, "GPJ13"))
