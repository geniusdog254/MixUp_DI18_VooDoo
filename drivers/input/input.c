...
//discard smile key(97) when the slide is not open.
#define DISCARD_SMILE_KEY
...
#ifdef DISCARD_SMILE_KEY
static int input_chk_discard(unsigned int code, int value)
{
    unsigned long key_status=1;

    //Discard smile key when the slide is not open.
    //low:closed
    if(system_rev >= 0x08){ 
...
//	pr_err("[key] type = %d, code = %d, value = %d \n", type, code, value);
}
