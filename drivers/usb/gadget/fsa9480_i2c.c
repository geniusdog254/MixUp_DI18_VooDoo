...
void Wimax_Switch_Config(int sel)
{
   ...
   DEBUG_FSA9480("[FSA9480]%s\n ", __func__);
		...
		s3c_gpio_cfgpin(GPIO_USB_HS_SW_EN_N, S5PC11X_GPJ2_OUTPUT(1));
   ...
   if((wimax_usb_state == USB_CABLE_50K) || Reserved2Status){
		...
		s3c_gpio_cfgpin(GPIO_USB_HS_SEL, S5PC11X_GPJ2_OUTPUT(0));
...
{	
		...
		return sprintf(buf, "%s[DMport test] Port test ready!! \n", buf);	
}
...
/*    Added source for the below issue.(20100405_inchul)
   ...
   else
   {
       ...
       {
           mdelay(10);
		        fsa9480_write(&fsa9480_i2c_client, REGISTER_MANUALSW1, 0x48);
       }
       else
       {
           mdelay(10);
        ...
        wake_lock(&cable_wake_lock);
			...
			case FSA9480_DEV_TY2_JIG_UART_ON:
				if(attach & FSA9480_INT1_ATTACH)
				{
					...
					//20100707_inchul... FSA9480_Enable_SPK(1);     
				}
				else
				{
...
void FSA9480_ReadIntRegister(void)
{
  ...
  if( (interrupt1 == 0x0) && (interrupt2 == 0x0) && (device1 == 0x0) && (device2 == 0x0) ){
	...
	if(interrupt1 & FSA9480_INT1_ATTACH)
	{
	 ...
	 fsa9480_write(&fsa9480_i2c_client, REGISTER_INTERRUPTMASK2, 0xFD);
#endif  
...
#if 1//CONFIG_ARIES_VER_B1... pfe
	 s3c_gpio_cfgpin( AP_SDA_30V, S5PC11X_GPD1_OUTPUT(2));
	 s3c_gpio_setpull( AP_SDA_30V, S3C_GPIO_PULL_UP);	 

	 s3c_gpio_cfgpin( AP_SCL_30V, S5PC11X_GPD1_OUTPUT(3) );
